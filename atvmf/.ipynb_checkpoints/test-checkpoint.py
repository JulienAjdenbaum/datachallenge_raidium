...
### save images ###
def Save_image(imgs, segs, anos, path):
    segs = np.argmax(segs, axis=1)
    for i in range(len(segs)):
        
        img = imgs[i]
        img = np.transpose(img, (1,2,0))
        seg = segs[i]
        np.save("{}_{}/Image/Seg_np/{}_{}.npy".format(args.out, args.model, path, str(i)), seg)
        ano = anos[i]
        ...
        img.save("{}_{}/Image/Inputs/{}_{}.png".format(args.out, args.model, path, str(i)), quality=95)
        dst1.save("{}_{}/Image/Seg/{}_{}.png".format(args.out, args.model, path, str(i)), quality=95)
        dst2.save("{}_{}/Image/Ano/{}_{}.png".format(args.out, args.model, path, str(i)), quality=95)

### test ###
def test():
    model_path = "{}_{}/model/model_bestdsc.pth".format(args.out, args.model)
    # model_path = "{}_{}/model/model_bestds.pth".format(args.out, args.model)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join("{}_{}".format(args.out, args.model), "Image", "Seg_np")):
      	os.mkdir(os.path.join("{}_{}".format(args.out, args.model), "Image", "Seg_np"))
